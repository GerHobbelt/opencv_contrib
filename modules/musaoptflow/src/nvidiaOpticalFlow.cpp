...
cv::Ptr<cv::musa::NvidiaOpticalFlow_1_0> cv::musa::NvidiaOpticalFlow_1_0::create
    (cv::Size, NVIDIA_OF_PERF_LEVEL, bool, bool, bool, int, Stream&, Stream&) {
    throw_no_musa(); return cv::Ptr<cv::musa::NvidiaOpticalFlow_1_0>(); }

cv::Ptr<cv::musa::NvidiaOpticalFlow_2_0> cv::musa::NvidiaOpticalFlow_2_0::create(
    ...
    throw_no_musa(); return cv::Ptr<cv::musa::NvidiaOpticalFlow_2_0>();
}
...
#define CUDA_MODULENAME TEXT("mtmusa.dll")
#elif defined(_WIN32)
...
{
private:
    typedef int(*PFNCudaCuCtxGetCurrent)(MUcontext*);
...
#if defined(_WIN32)
        m_cudaDriverAPIGetCurrentCtx = (PFNCudaCuCtxGetCurrent)GetProcAddress(m_hCudaModule, "muCtxGetCurrent");
#else
        m_cudaDriverAPIGetCurrentCtx = (PFNCudaCuCtxGetCurrent)dlsym(m_hCudaModule, "muCtxGetCurrent");
#endif
        if (!m_cudaDriverAPIGetCurrentCtx)
        {
            m_isFailed = true;
            CV_Error(Error::StsBadFunc,
                "Cannot find Cuda Driver API : muCtxGetCurrent() entry in Cuda library");
        }
...
class NvidiaOpticalFlowImpl : public cv::musa::NvidiaOpticalFlow_1_0
{
private:
    int m_width;
    int m_height;
    ...
    MUdeviceptr m_frame0cuDevPtr;
    MUdeviceptr m_frame1cuDevPtr;
    MUdeviceptr m_flowXYcuDevPtr;
    MUdeviceptr m_hintcuDevPtr;
    MUdeviceptr m_costcuDevPtr;
    ...
    m_gridSize(NV_OF_OUTPUT_VECTOR_GRID_SIZE_4)
{
    ...
    cuSafeCall(musaGetDeviceCount(&nGpu));
    if (m_gpuId < 0 || m_gpuId >= nGpu)
    {
        CV_Error(Error::StsBadArg, "Invalid GPU Ordinal");
    }

    cuSafeCall(musaSetDevice(m_gpuId));
    cuSafeCall(musaFree(m_cuContext));
    ...
    Stream& stream, InputArray hint, OutputArray cost)
{
    ...
    else if (_frame0.isGpuMat())
    {
        ...
        GpuMat __frame0 = _frame0.getMUSAGpuMat();
    ...
    {
        //Get GpuMats from InputArrays
        GpuMat __frame1 = _frame1.getMUSAGpuMat();
    ...
    if (m_enableExternalHints)
    {
        ...
        else if(hint.isGpuMat())
        {
            ...
            GpuMat _hint = hint.getMUSAGpuMat();
    ...
    int gridSize, InputOutputArray upsampledFlow)
{
    ...
    {
        GpuMat __flow = _flow.getMUSAGpuMat();
...
class NvidiaOpticalFlowImpl_2 : public cv::musa::NvidiaOpticalFlow_2_0
{
private:
    int m_width;
    int m_height;
    ...
    MUdeviceptr m_frame1cuDevPtr;
    MUdeviceptr m_flowXYcuDevPtr;
    MUdeviceptr m_flowXYUpScaledcuDevPtr;
    MUdeviceptr m_hintcuDevPtr;
    MUdeviceptr m_costcuDevPtr;
    ...
    Stream& stream, InputArray hint, OutputArray cost)
{
    ...
    if (m_scaleFactor > 1)
    {
        ...
        cv::musa::device::optflow_nvidia::FlowUpsample((void*)m_flowXYcuDevPtr, nSrcWidth, nSrcPitch,
...
Ptr<cv::musa::NvidiaOpticalFlow_1_0> cv::musa::NvidiaOpticalFlow_1_0::create(
...
Ptr<cv::musa::NvidiaOpticalFlow_2_0> cv::musa::NvidiaOpticalFlow_2_0::create(
    cv::Size imageSize, NVIDIA_OF_PERF_LEVEL perfPreset,
