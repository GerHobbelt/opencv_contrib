...
__global__ void InitLabeling(const musa::PtrStepSzb img,
                             musa::PtrStepSzi labels,
...
__global__ void Merge(musa::PtrStepSzi labels, unsigned char* last_pixel) {
  unsigned row = (blockIdx.y * kblock_rows + threadIdx.y) * 2;
  unsigned col = (blockIdx.x * kblock_cols + threadIdx.x) * 2;
  unsigned labels_index = row * (labels.step / labels.elem_size) + col;
...
__global__ void Compression(musa::PtrStepSzi labels) {
  unsigned row = (blockIdx.y * kblock_rows + threadIdx.y) * 2;
  unsigned col = (blockIdx.x * kblock_cols + threadIdx.x) * 2;
  unsigned labels_index = row * (labels.step / labels.elem_size) + col;
...
__global__ void FinalLabeling(const musa::PtrStepSzb img,
                              musa::PtrStepSzi labels) {
  unsigned row = (blockIdx.y * kblock_rows + threadIdx.y) * 2;
  unsigned col = (blockIdx.x * kblock_cols + threadIdx.x) * 2;
  unsigned labels_index = row * (labels.step / labels.elem_size) + col;
...
void BlockBasedKomuraEquivalence(const cv::musa::GpuMat& img,
                                 cv::musa::GpuMat& labels) {
  dim3 grid_size;
  dim3 block_size;
  unsigned char* last_pixel;
  ...
  if ((img.rows == 1 || img.cols == 1) && !((img.rows + img.cols) % 2)) {
    musaSafeCall(musaMalloc(&last_pixel, sizeof(unsigned char)));
    last_pixel_allocated = true;
  } else {
    last_pixel = labels.data + ((labels.rows - 2) * labels.step) +
                 (labels.cols - 2) * labels.elemSize();
  }

  grid_size = dim3((((img.cols + 1) / 2) - 1) / kblock_cols + 1,
                   (((img.rows + 1) / 2) - 1) / kblock_rows + 1, 1);
  ...
  if (last_pixel_allocated) {
    musaSafeCall(musaFree(last_pixel));
  }
