...
Ptr<musa::StereoSGM> cv::musa::createStereoSGM(int, int, int, int, int, int) { throw_no_musa(); return Ptr<musa::StereoSGM>(); }

#else /* !defined (HAVE_MUSA) */

#include "musa/stereosgm.hpp"
...
void StereoSGMImpl::compute(InputArray _left, InputArray _right, OutputArray _disparity, Stream& _stream)
{
    ...
    GpuMat left_disp = _disparity.getMUSAGpuMat();
...
Ptr<musa::StereoSGM> cv::musa::createStereoSGM(int minDisparity, int numDisparities, int P1, int P2, int uniquenessRatio, int mode)
{
