...
#include "opencv2/mudev/common.hpp"
#include "opencv2/mudev/warp/warp.hpp"
...
__device__ __forceinline__ static T shfl(T var, int srcLane, int width = mudev::WARP_SIZE, uint32_t mask = 0xFFFFFFFFU)
{
    return __shfl_sync(mask, var, srcLane, width);
}

template <unsigned int WARPS_PER_BLOCK, typename T>
__device__ __forceinline__ static T shfl_up(T var, unsigned int delta, int width = mudev::WARP_SIZE, uint32_t mask = 0xFFFFFFFFU)
{
    return __shfl_up_sync(mask, var, delta, width);
}

template <unsigned int WARPS_PER_BLOCK, typename T>
__device__ __forceinline__ static T shfl_down(T var, unsigned int delta, int width = mudev::WARP_SIZE, uint32_t mask = 0xFFFFFFFFU)
{
    return __shfl_down_sync(mask, var, delta, width);
}

template <unsigned int WARPS_PER_BLOCK, typename T>
__device__ __forceinline__ static T shfl_xor(T var, int laneMask, int width = mudev::WARP_SIZE, uint32_t mask = 0xFFFFFFFFU)
{
...
void censusTransform(const GpuMat& src, GpuMat& dest, Stream& _stream)
{
    ...
    const dim3 gdim(
        mudev::divUp(src.cols, width_per_block),
        mudev::divUp(src.rows, height_per_block));
    ...
    struct DynamicProgramming
{
        ...
        uint32_t *local_costs, uint32_t p1, uint32_t p2, uint32_t mask)
    {
        ...
        uint32_t lazy_out = 0, local_min = 0;
        {
            ...
            const uint32_t prev = detail::shfl_up<WARPS_PER_BLOCK>(dp[DP_BLOCK_SIZE - 1], 1, mudev::WARP_SIZE, mask);
        ...
        }
        {
            ...
            const uint32_t next = detail::shfl_down<WARPS_PER_BLOCK>(dp0, 1, mudev::WARP_SIZE, mask);
            uint32_t out = ::min(dp[k] - last_min, p2);
            out = ::min(out, dp[k - 1] - last_min + p1);
...
static constexpr unsigned int BLOCK_SIZE = mudev::WARP_SIZE * WARPS_PER_BLOCK;
    ...
    int min_disp)
{
    ...
    static const unsigned int SUBGROUPS_PER_WARP = mudev::WARP_SIZE / SUBGROUP_SIZE;
    static const unsigned int PATHS_PER_WARP =
        mudev::WARP_SIZE * DP_BLOCKS_PER_THREAD / SUBGROUP_SIZE;
    ...
    const unsigned int warp_id = mudev::Warp::warpId();
    const unsigned int group_id = mudev::Warp::laneId() / SUBGROUP_SIZE;
    const unsigned int lane_id = threadIdx.x % SUBGROUP_SIZE;
    ...
    Stream& _stream)
{
    ...
    const int gdim = mudev::divUp(left.rows, PATHS_PER_BLOCK);
    const int bdim = BLOCK_SIZE;
    musaStream_t stream = cv::musa::StreamAccessor::getStream(_stream);
...
{
    static const unsigned int SUBGROUP_SIZE = MAX_DISPARITY / DP_BLOCK_SIZE;
    static const unsigned int PATHS_PER_WARP = mudev::WARP_SIZE / SUBGROUP_SIZE;
    ...
    Stream& _stream)
{
    ...
    const int gdim = mudev::divUp(size.width, PATHS_PER_BLOCK);
    const int bdim = BLOCK_SIZE;
    musaStream_t stream = cv::musa::StreamAccessor::getStream(_stream);
    ...
    Stream& _stream)
{
    ...
    const Size size = left.size();
    const int gdim = mudev::divUp(size.width + size.height - 1, PATHS_PER_BLOCK);
    const int bdim = BLOCK_SIZE;
    musaStream_t stream = StreamAccessor::getStream(_stream);
...
namespace
{
static constexpr unsigned int WARPS_PER_BLOCK = 2u;
static constexpr unsigned int BLOCK_SIZE = WARPS_PER_BLOCK * mudev::WARP_SIZE;
...
__global__ void winner_takes_all_kernel(
    ...
    float uniqueness)
{
    static constexpr unsigned int ACCUMULATION_PER_THREAD = 16u;
    static constexpr unsigned int REDUCTION_PER_THREAD = MAX_DISPARITY / mudev::WARP_SIZE;
    static constexpr unsigned int ACCUMULATION_INTERVAL = ACCUMULATION_PER_THREAD / REDUCTION_PER_THREAD;
    static constexpr unsigned int UNROLL_DEPTH =
    ...
    const unsigned int warp_id = mudev::Warp::warpId();
    const unsigned int lane_id = mudev::Warp::laneId();
...
#pragma unroll
        for (unsigned int x1 = 0; x1 < UNROLL_DEPTH; ++x1)
        {
            if (x1 % ACCUMULATION_INTERVAL == 0)
            {
                ...
                __syncwarp(); // __threadfence_block(); for older version
            }
            ...
            if (x < width)
            {
                ...
                best = subgroup_min<WARPS_PER_BLOCK, mudev::WARP_SIZE>(best, 0xffffffffu);
                // Update right
#pragma unroll
                for (unsigned int i = 0; i < REDUCTION_PER_THREAD; ++i)
                {
                ...
                uniq = subgroup_and<WARPS_PER_BLOCK, mudev::WARP_SIZE>(uniq, 0xffffffffu);
                if (lane_id == 0)
                {
...
template <size_t MAX_DISPARITY>
void winnerTakesAll(const GpuMat& src, GpuMat& left, GpuMat& right, float uniqueness, bool subpixel, int mode, cv::musa::Stream& _stream)
{
    ...
    const int gdim = mudev::divUp(size.height, WARPS_PER_BLOCK);
    const int bdim = BLOCK_SIZE;
    musaStream_t stream = cv::musa::StreamAccessor::getStream(_stream);
...
template CV_EXPORTS void winnerTakesAll< 64>(const GpuMat&, GpuMat&, GpuMat&, float, bool, int, cv::musa::Stream&);
template CV_EXPORTS void winnerTakesAll<128>(const GpuMat&, GpuMat&, GpuMat&, float, bool, int, cv::musa::Stream&);
template CV_EXPORTS void winnerTakesAll<256>(const GpuMat&, GpuMat&, GpuMat&, float, bool, int, cv::musa::Stream&);
} // namespace winner_takes_all
...
{
    uint32_t mask = 0;
    reinterpret_cast<ushort *>(&mask)[0] = reinterpret_cast<ushort *>(&x)[0] > reinterpret_cast<ushort *>(&y)[0] ? 0xffff : 0x0000;
    reinterpret_cast<ushort *>(&mask)[1] = reinterpret_cast<ushort *>(&x)[1] > reinterpret_cast<ushort *>(&y)[1] ? 0xffff : 0x0000;
...
{
    uint32_t tmp = 0;
    reinterpret_cast<ushort *>(&tmp)[0] = reinterpret_cast<ushort *>(&x)[0] < reinterpret_cast<ushort *>(&y)[0] ? reinterpret_cast<ushort *>(&x)[0] : reinterpret_cast<ushort *>(&y)[0];
    reinterpret_cast<ushort *>(&tmp)[1] = reinterpret_cast<ushort *>(&x)[1] < reinterpret_cast<ushort *>(&y)[1] ? reinterpret_cast<ushort *>(&x)[1] : reinterpret_cast<ushort *>(&y)[1];
    x = tmp;
...
{
    uint32_t mask = 0;
    reinterpret_cast<uchar *>(&mask)[0] = reinterpret_cast<uchar *>(&x)[0] > reinterpret_cast<uchar *>(&y)[0] ? 0xff : 0x00;
    reinterpret_cast<uchar *>(&mask)[1] = reinterpret_cast<uchar *>(&x)[1] > reinterpret_cast<uchar *>(&y)[1] ? 0xff : 0x00;
    reinterpret_cast<uchar *>(&mask)[2] = reinterpret_cast<uchar *>(&x)[2] > reinterpret_cast<uchar *>(&y)[2] ? 0xff : 0x00;
    reinterpret_cast<uchar *>(&mask)[3] = reinterpret_cast<uchar *>(&x)[3] > reinterpret_cast<uchar *>(&y)[3] ? 0xff : 0x00;
...
{
    uint32_t tmp = 0;
    reinterpret_cast<uchar *>(&tmp)[0] = reinterpret_cast<uchar *>(&x)[0] < reinterpret_cast<uchar *>(&y)[0] ? reinterpret_cast<uchar *>(&x)[0] : reinterpret_cast<uchar *>(&y)[0];
    reinterpret_cast<uchar *>(&tmp)[1] = reinterpret_cast<uchar *>(&x)[1] < reinterpret_cast<uchar *>(&y)[1] ? reinterpret_cast<uchar *>(&x)[1] : reinterpret_cast<uchar *>(&y)[1];
    reinterpret_cast<uchar *>(&tmp)[2] = reinterpret_cast<uchar *>(&x)[2] < reinterpret_cast<uchar *>(&y)[2] ? reinterpret_cast<uchar *>(&x)[2] : reinterpret_cast<uchar *>(&y)[2];
    reinterpret_cast<uchar *>(&tmp)[3] = reinterpret_cast<uchar *>(&x)[3] < reinterpret_cast<uchar *>(&y)[3] ? reinterpret_cast<uchar *>(&x)[3] : reinterpret_cast<uchar *>(&y)[3];
    x = tmp;
...
void median_filter<uint8_t>(const PtrStepSz<uint8_t> d_src, PtrStep<uint8_t> d_dst, Stream& _stream)
{
    ...
    if ((d_src.step / sizeof(uint8_t)) % 4 == 0)
    {
        ...
        const dim3 grid(mudev::divUp(d_src.cols / 4, block.x), mudev::divUp(d_src.rows, block.y));
        median_kernel_3x3_8u_v4<<<grid, block, 0, stream>>>(d_src, d_dst);
    }
    else
    {
        const dim3 block(BLOCK_X, BLOCK_Y);
        const dim3 grid(mudev::divUp(d_src.cols, block.x), mudev::divUp(d_src.rows, block.y));
        median_kernel_3x3_8u<<<grid, block, 0, stream>>>(d_src, d_dst);
    }

    CV_MUDEV_SAFE_CALL(musaGetLastError());
}
...
void median_filter(const PtrStepSz<uint16_t> d_src, PtrStep<uint16_t> d_dst, Stream& _stream)
{
    ...
    {
        const dim3 block(BLOCK_X, BLOCK_Y);
        const dim3 grid(mudev::divUp(d_src.cols / 2, block.x), mudev::divUp(d_src.rows, block.y));
    ...
    Stream& _stream)
{
    ...
    const dim3 blocks(mudev::divUp(size.width, SIZE), mudev::divUp(size.height, SIZE));
