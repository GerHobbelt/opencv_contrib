...
public:
    inline OmnidirTest()
        ...
        , K(9.3427053703903516e+02, 0., 6.1854878705698127e+02, 0.,
            9.3683593862086059e+02, 3.7989519649147530e+02,
            0., 0., 1.)
...
protected:
    void run(int) override
    {
        ...
        cv::omnidir::calibrate(objectPoints, imagePoints, imageSize, estK, estXi, estD,
...
protected:
    void run(int) override
    {
        ...
        // scale0 = 0, scale1 = 1
        progress = update_progress( progress, 1, 23, 0 );
        cv::Matx33d K_new_truth = cv::Matx33d(  4.5288365257994622e+02, 0., 5.8291094241366773e+02,
                                                0., 4.5412722003996015e+02, 3.7066458811112551e+02,
                                                0., 0., 1.);
        runCasePerspective(++caseId, K_new_truth, R_eye, 0.0, 1.0);
        // scale0 = -1, scale1 = -1
        progress = update_progress( progress, 2, 23, 0 );
        runCasePerspective(++caseId, K_new_truth, R_eye, -1.0, -1.0);
        // scale0 = 0, scale1 = 0
        progress = update_progress( progress, 3, 23, 0 );
        runCasePerspective(++caseId, K_new_truth, R_eye, 0.0, 0.0);
        // scale0 = 1, scale1 = 1
        progress = update_progress( progress, 4, 23, 0 );
        K_new_truth = cv::Matx33d(  2.7199168328349276e+02, 0., 6.0571360264050668e+02,
                                    0., 2.7273854178633098e+02, 3.8238181481266639e+02,
                                    0., 0., 1.);
        runCasePerspective(++caseId, K_new_truth, R_eye, 1.0, 1.0);
        // scale0 = 1, scale1 = 0
        progress = update_progress( progress, 5, 23, 0 );
        runCasePerspective(++caseId, K_new_truth, R_eye, 1.0, 0.0);
        // scale0 = 0.5, scale1 = 0.5
        progress = update_progress( progress, 6, 23, 0 );
        K_new_truth = cv::Matx33d( 7.2487533586343898e+02, 0., 5.4862454505417429e+02,
                                        0., 7.2686576182629119e+02, 3.5304640292379185e+02,
                                        0., 0., 1.);
        runCasePerspective(++caseId, K_new_truth, R_eye, 0.5, 0.5);
        // scale0 = 0.0, scale1 = 0.5
        progress = update_progress( progress, 7, 23, 0 );
        K_new_truth = cv::Matx33d( 9.0576730515989243e+02, 0., 5.2582188482733534e+02,
                                        0., 9.0825444007992030e+02, 3.4132917622225096e+02,
                                        0., 0., 1.);
        runCasePerspective(++caseId, K_new_truth, R_eye, 0.0, 0.5);
        // scale0 = 1, scale1 = 0.5
        progress = update_progress( progress, 8, 23, 0 );
        K_new_truth = cv::Matx33d( 5.4398336656698552e+02, 0., 5.7142720528101324e+02,
                                        0., 5.4547708357266197e+02, 3.6476362962533273e+02,
                                        0., 0., 1.);
        runCasePerspective(++caseId, K_new_truth, R_eye, 1.0, 0.5);
        // scale0 = 0.5, scale1 = 1.0
        progress = update_progress( progress, 9, 23, 0 );
        K_new_truth = cv::Matx33d( 3.6243766793171949e+02, 0., 5.9431227252708709e+02,
                                        0., 3.6343288091314560e+02, 3.7652320146189595e+02,
                                        0., 0., 1.);
        runCasePerspective(++caseId, K_new_truth, R_eye, 0.5, 1.0);
        // scale0 = 0.0, scale1 = 2
        progress = update_progress( progress, 10, 23, 0 );
        K_new_truth = cv::Matx33d( 2.2644182628997311e+02, 0., 6.1145547120683386e+02,
                                        0., 2.2706361001998008e+02, 3.8533229405556278e+02,
                                        0., 0., 1. );
        runCasePerspective(++caseId, K_new_truth, R_eye, 0.0, 2.0);
        // scale0 = 1, scale1 = 2
        progress = update_progress( progress, 11, 23, 0 );
        K_new_truth = cv::Matx33d( 1.3599584164174638e+02, 0., 6.2285680132025334e+02,
                    0., 1.3636927089316549e+02, 3.9119090740633317e+02,
                    0., 0., 1. );
        runCasePerspective(++caseId, K_new_truth, R_eye, 1.0, 2.0);
        // scale0 = 2, scale1 = 2
        progress = update_progress( progress, 12, 23, 0 );
        runCasePerspective(++caseId, K_new_truth, R_eye, 2.0, 2.0);
        // scale0 = 0.5, scale1 = 2
        progress = update_progress( progress, 13, 23, 0 );
        K_new_truth = cv::Matx33d( 1.8121883396585974e+02, 0., 6.1715613626354354e+02,
                    0., 1.8171644045657280e+02, 3.8826160073094798e+02,
                    0., 0., 1. );
        runCasePerspective(++caseId, K_new_truth, R_eye, 0.5, 2.0);


        // scale0 = 0, scale1 = 1
        progress = update_progress( progress, 14, 23, 0 );
        K_new_truth = cv::Matx33d(5.5459064803950560e+02, 0., -2.4965993983377021e+02,
                                    0., 5.5347894441196308e+02, -4.8857633821260163e+02,
                                    0., 0., 1.);
        runCaseLongiLati(++caseId, K_new_truth, R_eye, 0.0, 1.0);
        // scale0 = -1, scale1 = -1
        progress = update_progress( progress, 15, 23, 0 );
        runCaseLongiLati(++caseId, K_new_truth, R_eye, -1.0, -1.0);
        // scale0 = 0, scale1 = 0
        progress = update_progress( progress, 16, 23, 0 );
        runCaseLongiLati(++caseId, K_new_truth, R_eye, 0.0, 0.0);
        // scale0 = 1, scale1 = 1
        progress = update_progress( progress, 17, 23, 0 );
        runCaseLongiLati(++caseId, K_new_truth, R_eye, 1.0, 1.0);
        // scale0 = 1, scale1 = 0
        progress = update_progress( progress, 18, 23, 0 );
        runCaseLongiLati(++caseId, K_new_truth, R_eye, 1.0, 0.0);
        // scale0 = 0.5, scale1 = 0.5
        progress = update_progress( progress, 19, 23, 0 );
        K_new_truth = cv::Matx33d( 1.1091812960790112e+03, 0., -1.1208088926490268e+03,
                0., 1.1069578888239262e+03, -1.3579790310532298e+03,
                0., 0., 1. );
        runCaseLongiLati(++caseId, K_new_truth, R_eye, 0.5, 0.5);
        // scale0 = 1.0, scale1 = 0.5
        progress = update_progress( progress, 20, 23, 0 );
        K_new_truth = cv::Matx33d( 5.5459064803950560e+02, 0., -2.4965993983377021e+02, 0.,
       1.1069578888239262e+03, -1.3579790310532298e+03, 0., 0., 1. );
        runCaseLongiLati(++caseId, K_new_truth, R_eye, 1.0, 0.5);
        // scale0 = 0.5, scale1 = 1.0
        progress = update_progress( progress, 21, 23, 0 );
        K_new_truth = cv::Matx33d( 1.1091812960790112e+03, 0., -1.1208088926490268e+03, 0.,
       5.5347894441196308e+02, -4.8857633821260163e+02, 0., 0., 1. );
        runCaseLongiLati(++caseId, K_new_truth, R_eye, 0.5, 1.0);
        // scale0 = 2.0, scale1 = 2.0
        progress = update_progress( progress, 22, 23, 0 );
        K_new_truth = cv::Matx33d( 2.7729532401975280e+02, 0., 1.8591453657385813e+02, 0.,
       2.7673947220598154e+02, -5.3874991792287453e+01, 0., 0., 1. );
        runCaseLongiLati(++caseId, K_new_truth, R_eye, 2.0, 2.0);
        progress = update_progress( progress, 23, 23, 0 );
    }

    // test accuracy of calculating new camera matrix for undistort rectify width different rotations, and scaling parameters for perspective rectification
    void runCasePerspective(int caseId, cv::Matx33d K_new_truth, cv::Mat R, double scale0, double scale1)
    {
        ...
        cv::omnidir::estimateNewCameraMatrixForUndistortRectify(this->K, this->D, this->Xi, this->imageSize, R, K_new, cv::omnidir::RECTIFY_PERSPECTIVE, scale0, scale1);
        ...
        if (!result) {
            ts->printf(cvtest::TS::LOG, "case %d: rectification: perspective, scale0=%lf, scale1=%lf: %s\n",
                    caseId, scale0, scale1, result.message());
            ts->set_failed_test_info(cvtest::TS::FAIL_MISMATCH);
            return;
        }
    }

    // test accuracy of calculating new camera matrix for undistort rectify width different rotations, and scaling parameters for longitute latitude rectification
    void runCaseLongiLati(int caseId, cv::Matx33d K_new_truth, cv::Mat R, double scale0, double scale1)
    {
        cv::Mat K_new(3, 3, cv::DataType<double>::type);

        cv::omnidir::estimateNewCameraMatrixForUndistortRectify(this->K, this->D, this->Xi, this->imageSize, R, K_new, cv::omnidir::RECTIFY_LONGLATI, scale0, scale1);
        auto result = cvtest::assertMatNear("K_new", "K_new_truth", "1e-6", K_new, K_new_truth, 1e-6);
...
TEST(CV_CalibrationOmni, omnidir) { CV_CalibrationOmni test; test.safe_run(); }
TEST(CV_EstimateNewCameraMatrixForUndistortRectify, omnidir) { CV_EstimateNewCameraMatrixForUndistortRectify test; test.safe_run(); }
