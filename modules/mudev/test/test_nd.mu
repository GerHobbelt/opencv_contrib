...
class GpuMatNDTest : public ::testing::Test {
 public:
  ...
  using SizeArray = cv::musa::GpuMatND::SizeArray;
  ...
  static MatType RandomMat(const SizeArray& size) {
    ...
    for (ElemType& elem : ret)
      for (int i = 0; i < cn; ++i) elem[i] = cv::randu<CnType>();
  ...
  static void doTest3(const SizeArray& size) {
    if (std::is_same<CnType, float16_t>::value)  // GpuMat::convertTo is not
                                                 // implemented for CV_16F
      return;
    ...
    gmat.upload(gold);
    cv::musa::GpuMat plane = gmat(ranges).createGpuMatHeader();
    EXPECT_TRUE(!plane.refcount);  // plane points to externally allocated
                                   // memory(a part of gmat)

    const cv::musa::GpuMat dummy = plane.clone();
    EXPECT_TRUE(dummy.refcount);  // dummy is clone()-ed from plane, so it
                                  // manages its memory
    ...
    EXPECT_TRUE(!plane.refcount);  // plane still points to externally allocated
                                   // memory(a part of gmat)
  ...
  static void doTest4(const SizeArray& size) {
    ...
    gmat.upload(gold);
    const cv::musa::GpuMatND external(gmat.size, gmat.type(),
                                      gmat.getDevicePtr(),
                                      {gmat.step.begin(), gmat.step.end() - 1});

    // set a sub-matrix region of external to 2
    cv::musa::GpuMat plane = external(ranges).createGpuMatHeader();
    const cv::musa::GpuMat dummy = plane.clone();
  ...
  static void doTest5(const SizeArray& size) {
    ...
    // Upload a sub-mat, set a sub-region of the sub-mat to 3, download, and
    // compare
    gmat.upload(goldSub);
    const std::vector<Range> rangesInRanges = RandomRange2D(gmat.size);

    cv::musa::GpuMat plane = gmat(rangesInRanges).createGpuMatHeader();
    const cv::musa::GpuMat dummy = plane.clone();
...
using ElemTypes = ::testing::Types<
    ...
    Vec<float16_t, 1>, Vec<float16_t, 2>, Vec<float16_t, 3>,
    Vec<float16_t, 4>  // CV_16F
    >;

using SizeArray = cv::musa::GpuMatND::SizeArray;

#define DIFFERENT_SIZES_ND                                                   \
  std::vector<SizeArray> {                                                   \
    SizeArray{2, 1}, SizeArray{3, 2, 1}, SizeArray{1, 3, 2, 1},              \
        SizeArray{2, 1, 3, 2, 1}, SizeArray{3, 2, 1, 3, 2, 1}, SizeArray{1}, \
        SizeArray{1, 1}, SizeArray{1, 1, 1}, SizeArray{1, 1, 1, 1},          \
        SizeArray{4}, SizeArray{4, 4}, SizeArray{4, 4, 4},                   \
        SizeArray{4, 4, 4, 4}, SizeArray{11}, SizeArray{13, 11},             \
        SizeArray{17, 13, 11}, SizeArray {                                   \
      19, 17, 13, 11                                                         \
    }                                                                        \
  }
...
TYPED_TEST(GpuMatNDTest, Test1) {
  for (auto& size : DIFFERENT_SIZES_ND) GpuMatNDTest<TypeParam>::doTest1(size);
}

TYPED_TEST(GpuMatNDTest, Test2) {
  for (auto& size : DIFFERENT_SIZES_ND) GpuMatNDTest<TypeParam>::doTest2(size);
}

TYPED_TEST(GpuMatNDTest, Test3) {
  for (auto& size : DIFFERENT_SIZES_ND) GpuMatNDTest<TypeParam>::doTest3(size);
}

TYPED_TEST(GpuMatNDTest, Test4) {
  for (auto& size : DIFFERENT_SIZES_ND) GpuMatNDTest<TypeParam>::doTest4(size);
}

TYPED_TEST(GpuMatNDTest, Test5) {
  for (auto& size : DIFFERENT_SIZES_ND) GpuMatNDTest<TypeParam>::doTest5(size);
}
